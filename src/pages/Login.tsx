...
const Login = () => {
  ...
  const handleWorkplaceSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from("workplaces").select("id, name, company_name").eq("workplace_code", workplaceCode.toUpperCase()).maybeSingle();
      if (error || !data) {
  ...
  const handlePersonalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === "login") {
        ...
        const {
          error
        } = await signUp(email, password, fullName, workplaceId);
        if (error) {
          toast({
            variant: "destructive",
  ...
  return <div className="min-h-screen wb-gradient-hero flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-card rounded-2xl wb-shadow-elevated p-8">
          ...
          {step === "workplace" ? <form onSubmit={handleWorkplaceSubmit} className="space-y-6 animate-fade-in">
              ...
              <div className="space-y-2">
                ...
                <Input id="workplaceCode" value={workplaceCode} onChange={e => {
              setWorkplaceCode(e.target.value.toUpperCase());
              setCodeError("");
            }} placeholder="T.ex. NNS-SOLNA" className={`h-12 text-center text-lg tracking-wider font-mono ${codeError ? "border-destructive" : ""}`} required />
                {codeError && <p className="text-sm text-destructive">{codeError}</p>}
              </div>

              <Button type="submit" variant="hero" size="lg" className="w-full" disabled={isLoading || !workplaceCode}>
                {isLoading ? <div className="h-5 w-5 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" /> : <>
                    Fortsätt
              ...
              <div className="flex rounded-lg bg-secondary p-1 mb-4">
                <button type="button" onClick={() => setAuthMode("login")} className={`flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors ${authMode === "login" ? "bg-card wb-shadow-soft text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
                  Logga in
                </button>
                <button type="button" onClick={() => setAuthMode("signup")} className={`flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors ${authMode === "signup" ? "bg-card wb-shadow-soft text-foreground" : "text-muted-foreground hover:text-foreground"}`}>
                  Skapa konto
                </button>
              </div>
              ...
              <div className="space-y-4">
                {authMode === "signup" && <div className="space-y-2">
                    ...
                    <Input id="fullName" value={fullName} onChange={e => setFullName(e.target.value)} placeholder="Ditt namn" className="h-12" required />
                  </div>}

                <div className="space-y-2">
                  <Label htmlFor="email">E-post</Label>
                  <Input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="din@email.se" className="h-12" required />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="password">Lösenord</Label>
                  <Input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" className="h-12" required minLength={6} />
                </div>
              </div>

              <div className="space-y-3">
                <Button type="submit" variant="hero" size="lg" className="w-full" disabled={isLoading}>
                  {isLoading ? <div className="h-5 w-5 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" /> : authMode === "login" ? "Logga in" : "Skapa konto"}
                </Button>

                <button type="button" onClick={() => setStep("workplace")} className="w-full text-sm text-muted-foreground hover:text-foreground transition-colors">
                  ← Byt arbetsplats
                </button>
              </div>
