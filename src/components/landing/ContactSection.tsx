...
export const ContactSection = () => {
  ...
  const [formData, setFormData] = useState({
    company: "", contactPerson: "", email: "", phone: "", workplaceType: "", message: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("contact_leads").insert({
        company: formData.company, contact_person: formData.contactPerson, email: formData.email,
        phone: formData.phone || null, workplace_type: formData.workplaceType || null, message: formData.message || null,
      });
      if (error) throw error;
      await supabase.functions.invoke("send-contact-notification", {
        body: { company: formData.company, contactPerson: formData.contactPerson, email: formData.email, phone: formData.phone || null, workplaceType: formData.workplaceType || null, message: formData.message || null },
      }).catch(console.error);
      setIsSubmitted(true);
      toast({ title: "Tack för ditt intresse!", description: "Vi återkommer inom kort." });
      setTimeout(() => { setFormData({ company: "", contactPerson: "", email: "", phone: "", workplaceType: "", message: "" }); setIsSubmitted(false); }, 3000);
    } catch {
        ...
        <Reveal>
          <div className="max-w-lg mx-auto">
            <div className="text-center mb-10">
              <h2 className="text-2xl sm:text-3xl font-bold text-foreground tracking-tight mb-3">Boka en demo</h2>
              <p className="text-[14px] text-muted-foreground">Se hur WorkBuddy förbättrar internkommunikation och personalhantering.</p>
            </div>
            {isSubmitted ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Button type="submit" className="w-full h-10 rounded-lg text-[13px] group" disabled={isSubmitting}>
                  {isSubmitting ? (
                    <div className="h-4 w-4 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" />
                  ) : (
                    <>Boka genomgång<ArrowRight className="h-3.5 w-3.5 ml-1.5 transition-transform group-hover:translate-x-0.5" /></>
                  )}
                </Button>
              </form>
            )}
